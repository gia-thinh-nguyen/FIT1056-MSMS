...
def check_in(student_id, course_id, timestamp=None):
    ...
    timestamp: The time of check-in (str, optional)
    """
...
def print_student_card(student_id):
    ...
    for s in app_data['students']:
        if s['id'] == student_id: 
            student_to_print = s # When found, assign it to student_to_print then break the loop.
            break
    
    if student_to_print: # If there is matched student
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            update_teacher(teacher_id, name=name, speciality=speciality)
            made_change = True
        elif choice == '4':
